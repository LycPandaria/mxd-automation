...
1. 确保游戏已启动，窗口标题为 `MXD怀旧服`（或其他你在配置中设置的名字）
2. 点击顶部 **「刷新列表」** 按钮，下拉框会自动列出所有可见窗口
3. 在下拉框中选择游戏窗口，点击 **「锁定窗口」**
4. 锁定成功后顶部状态变为绿色 **「已锁定: MXD怀旧服」**
...
- **窗口标题**必须与配置中的 `window_title` 一致（默认 `MXD怀旧服`），否则程序找不到游戏窗口。
- **窗口必须保持前台激活状态**（最上层），因为MXD通过 `DirectInput` 读取全局键盘状态，SendInput 的按键只有窗口在前台时才能被游戏接收。运行期间不能切换去操作其他程序。
...
使用 Win32 `SendInput` API 从驱动层模拟真实全局按键。MXD通过 `DirectInput` / `GetAsyncKeyState` 读取全局键盘状态，**只有此模式能正常工作**。
