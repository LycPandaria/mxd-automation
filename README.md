...
| `monster_classes` | `monster` | 怪物类别名（逗号分隔） |
| `floor_classes` | `floor` | 地板类别名（逗号分隔） |
| `rope_classes` | `rope` | 绳索类别名（逗号分隔） |
| `player_classes` | `player` | 玩家类别名（用于自身定位，逗号分隔） |
| `attack_range` | `200` | 攻击范围（像素），长手专用（exe 界面"攻击距离px"可修改）；短手固定 50px 不读此配置 |
| `attack_range_y` | `60` | 同平台垂直容差（像素） |
